...
for k = 1:pvz_eiluciu_sk
    startIdx = round((k-1)*raidziu_sk+1);
    endIdx = round(k*raidziu_sk);

    O_centras(startIdx:endIdx, :) = ...
        sortrows(O_centras(startIdx:endIdx, :), 3);
end
...
for k = 1:Skaicius
    subplot(pvz_eiluciu_sk, ceil(Skaicius/pvz_eiluciu_sk), k);
    imshow(objektai{k});
end

% Ensure raidziu_sk is an integer and not zero
raidziu_sk = ceil(Skaicius / pvz_eiluciu_sk); % Use ceil to ensure raidziu_sk is an integer
if raidziu_sk == 0
    error('raidziu_sk is zero, which is invalid for subplot. Ensure that Skaicius is not zero and pvz_eiluciu_sk is a valid divisor.');
end
...
try
    for k = 1:Skaicius
        % Calculate subplot's row and column for the current index k
        row = ceil(k / raidziu_sk);
        col = mod(k - 1, raidziu_sk) + 1;
        
        % Ensure that row and column indices are valid for subplot
        if row > pvz_eiluciu_sk
            error('Row index exceeds the number of symbol lines (pvz_eiluciu_sk).');
        end
        if col > raidziu_sk
            error('Column index exceeds the calculated number of symbols per line (raidziu_sk).');
        end

        % Use subplot to display each image segment
        subplot(pvz_eiluciu_sk, raidziu_sk, (row - 1) .* raidziu_sk + col);
        imshow(objektai{k});
    end
catch err
    disp(err.message);
